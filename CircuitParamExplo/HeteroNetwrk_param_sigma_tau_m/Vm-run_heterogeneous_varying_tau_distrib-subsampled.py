...
# Setup heterogeneous Threshold and Vm0
...
for sigma in [1, 2, 3]:  # sigma = 1,2,3  standard deviation of tau_m, the param to change !!
    ...
    for idx, lag in zip(indices, time_lags):
        ...
        with open('resultsNEST_param_HN_sigma_{0}.csv'.format(str(sigma)), 'a') as res_file:
            res_file.write('{0},{1},{2},{3},{4},{5}\n'.format(
                str(sigma),str(lag), str(enc_capacity), str(enc_error), str(circ_capacity), str(circ_error)))
    ...
    ax1.plot(time_lags*(subsampling_factor), encoder_capacity)
    ax1.set_title('Total encoder capacity: \n {0} ms'.format(str(tot_cap_enc)))
    ax2.plot(time_lags*(subsampling_factor), circuit_capacity)
