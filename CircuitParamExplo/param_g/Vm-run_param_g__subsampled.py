...
for g in [0.5, 1, 2.5, 5, 10, 20]:
    ...
    w_mat.savefig('Weight_matrice_g_{0}.png'.format(str(g)))
    ##
    ...
    inds_subsampling=np.arange(ind_init,len(signal)+
                               ind_init,subsampling_factor,dtype=int)
    ...
    for idx, lag in zip(indices, time_lags):
        ...
        with open('resultsNEST_param_g_{0}.csv'.format(str(g)), 'a') as res_file:
            res_file.write('{0},{1},{2},{3},{4},{5}\n'.format(
                str(g),str(lag*subsampling_factor), str(enc_capacity), str(enc_error), str(circ_capacity), str(circ_error)))
    ...
    tot_cap_circ = round(np.sum(circuit_capacity*subsampling_factor)*step_lag)    
    # Get plot
    fig = pl.figure()
    ax1 = fig.add_subplot(121)
    ax2 = fig.add_subplot(122)

    ax1.plot(time_lags*subsampling_factor, encoder_capacity,'#A444AD')
    ax1.set_title('Total encoder capacity: \n {0} ms'.format(str(tot_cap_enc)))
    ax2.plot(time_lags*subsampling_factor, circuit_capacity,'#B9B444')
    ax2.set_title("Total processor capacity: \n {0} ms".format(str(tot_cap_circ)))
    ...
    fig.savefig('MemCapacity_with_g_{0}.png'.format(str(g)))